 ...
 *
 * <p>This class provides the managed classes wrapped in {@link ManagedContainer}s.
 */
@Slf4j
public class ManagedEntitiesRepository {
     ...
     */
    private final Map<String, ManagedContainer<?>> singletons;

    /**
     * Map of all the multi instance managed classes. Each of them is a list, containing all the
     * instances of the managed type, wrapped in {@link ManagedContainer}s.
     */
    private final Map<String, List<ManagedContainer<?>>> multiInstances;
    ...
    private void checkNameUniqueness(String name) throws NameNotUniqueException {
        ...
        multiInstances
                .keySet()
                .forEach(
                        multiInstanceBaseName -> {
                            if (name.equals(multiInstanceBaseName))
